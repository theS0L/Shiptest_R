...
/obj/item/ammo_box/attackby(obj/item/attacking_obj, mob/user, params, silent = FALSE, replace_spent = FALSE)
	...
	if(istype(attacking_obj, /obj/item/ammo_box/magazine/ammo_stack))
		...
		for(var/obj/item/ammo_casing/casing_to_insert in attacking_box.stored_ammo)
			...
			// if(!((instant_load && attacking_box.instant_load) || (stored_ammo.len >= max_ammo) || do_after(user, 1 SECONDS, attacking_box, timed_action_flags = IGNORE_USER_LOC_CHANGE))) // CELADON-EDIT - ORIGINAL
			if(!((instant_load && attacking_box.instant_load) || (stored_ammo.len >= max_ammo) || do_after(user, 0.6 SECONDS, attacking_box, timed_action_flags = IGNORE_USER_LOC_CHANGE)))
			// [/CELADON-EDIT]
				break
