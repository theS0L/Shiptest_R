...
/datum/overmap/ship/controlled
		...
		// var/datum/newscaster/feed_network/newscaster

/datum/overmap/ship/controlled/proc/SendSos(name, x, y)
	if((!COOLDOWN_FINISHED(src, sendsos_cooldown)))
		return FALSE
	priority_announce("Наши локаторы получили сигнал бедствия S.O.S., который исходит от судна '" + name + "' по координатам: X - " + x + " / Y - " + y + ". " + name + " запрашивает скорейшей помощи!", sound = 'mod_celadon/sound/announce/sos.ogg', sender_override = "Outpost Distress Beacon System", zlevel = 0)
	COOLDOWN_START(src, sendsos_cooldown, 45 MINUTES)
// Отправляем сигнал в Wideband.
	radio = new(src)
	radio.talk_into(src, "Был активирован сигнал бедствия S.O.S. на координатах: X - " + x + " / Y - " + y + ".", FREQ_WIDEBAND)
// Делаем оповещение для Newscaster - Новые новости в системе Гриффон!
	...
	if(CONFIG_GET(string/servername) == "\[RU] Celadon Shiptest: Alpha")
		send2chat(":Celadon: \[Shiptest: Alpha] > Внимание! Судно **" + name + "** подаёт сигнал бедствия **S.O.S.** на координатах: **X - " + x + "** / **Y - " + y + "**.", "shiptest-chat")
	if(CONFIG_GET(string/servername) == "\[RU] Celadon Shiptest: Beta")
		send2chat(":Celadon: \[Shiptest: Beta] > Внимание! Судно **" + name + "** подаёт сигнал бедствия **S.O.S.** на координатах: **X - " + x + "** / **Y - " + y + "**.", "shiptest-chat")
// Логируем действия.
